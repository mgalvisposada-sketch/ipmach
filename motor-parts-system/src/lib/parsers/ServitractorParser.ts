...
export class ServitractorParser extends BaseParser {
        ...
        if (jsonMatch) {
          ...
          if (isHtml && (trimmed.includes('MES Login') || trimmed.includes('<title>MES Login</title>') ||
            ...
            return {
              ...
              metadata: {
                totalFound: 0,
                error: 'Received login page - cookies may have expired or authentication failed. Please update cookies in the database.',
              },
            };
          }
  ...
  private parseHtmlTiles(html: string, searchTerm: string): ParseResult {
    ...
    // Find all tile cards that contain product data (skip header/title cards)
    // Product tiles have columns with specific IDs like pc_13, pc_18, pc_23, pc_28, pc_33 (Código, Descripción, Existencias, Precio columns)
    ...
    // Filter to only product tiles - they contain the columns with Código, Descripción, etc.
    const productTiles = allTileCards.filter((_, card) => {
      const $card = $(card);
      // Product tiles have columns with the label structure (Código, Descripción, Existencias, Precio)
      const hasCodigoLabel = $card.find('span[texttype="Text"]:contains("Código")').length > 0;
      const hasDescripcionLabel = $card.find('span[texttype="Text"]:contains("Descripción")').length > 0;
      return hasCodigoLabel && hasDescripcionLabel;
    });
    ...
    productTiles.each((index, cardElement) => {
      try {
        ...
        // Find all spans with value attributes (these contain the actual data)
        const valueSpans = $card.find('span[text="true"][value]');
        ...
        // Also find label spans to map to values
        const labelSpans = $card.find('span[texttype="Text"]');

        // Create a map of label to value by finding the value span that comes after each label
        labelSpans.each((_, labelSpan) => {
          ...
          // Structure: The outer .zcp-col with col-width="fill" contains two .zcp-row elements
          // First row has the label (in nested divs), second row has the value (in nested divs)
          // The label span might be deeply nested, so we need to go up several levels
          let $labelColumn = $label.closest('.zcp-col').parent().closest('.zcp-col').parent().closest('.zcp-col');

          // Try different parent levels if the first attempt doesn't work
          if ($labelColumn.length === 0 || $labelColumn.attr('col-width') === undefined) {
            // Try going up to find a column with col-width attribute
            $labelColumn = $label.closest('[col-width]');
            console.log(`🔍 [SERVITRACTOR] Tile ${index + 1} - Found parent column with col-width="${$labelColumn.attr('col-width')}"`);
          }

          if ($labelColumn.length === 0) {
            console.log(`🔍 [SERVITRACTOR] Tile ${index + 1} - No parent column found for label "${labelText}"`);
            return;
          }

          // Find all variable spans in this parent column (these are the values)
          const $valueSpans = $labelColumn.find('span[texttype="variable"] span[text="true"][value]');

          console.log(`🔍 [SERVITRACTOR] Tile ${index + 1} - Found ${$valueSpans.length} value spans for label "${labelText}"`);

          // Debug: Print column structure
          if ($valueSpans.length === 0) {
            console.log(`🔍 [SERVITRACTOR] Tile ${index + 1} - DEBUG: Column HTML preview:`, $labelColumn.html()?.substring(0, 300));
          }
          ...
          if ($valueSpans.length > 0) {
            ...
            if (labelText === 'Código') {
              ...
              // Parse price: handle format like "$78,908.51" or "$78.908,51"
              let cleanPrice = value.replace(/[$\s]/g, '');

              // Check format: if contains both dot and comma
              if (cleanPrice.includes('.') && cleanPrice.includes(',')) {
                const lastDotIndex = cleanPrice.lastIndexOf('.');
                const lastCommaIndex = cleanPrice.lastIndexOf(',');

                if (lastCommaIndex > lastDotIndex) {
                  // European format: "78.908,51" (dot = thousands, comma = decimal)
                  cleanPrice = cleanPrice.replace(/\./g, '').replace(',', '.');
                } else {
                  // US format: "78,908.51" (comma = thousands, dot = decimal)
                  cleanPrice = cleanPrice.replace(/,/g, '');
                }
              } else if (cleanPrice.includes(',')) {
                // Only comma: could be "78908,51" (European decimal)
                cleanPrice = cleanPrice.replace(',', '.');
              }
